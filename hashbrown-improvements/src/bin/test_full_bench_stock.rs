/// Same benchmark as test_full_bench but using std::collections::HashMap
/// (stock hashbrown baked into stdlib) for comparison.
...
struct KeyData { keys: Vec<[u8; 16]>, miss_keys: Vec<[u8; 16]>, order: Vec<usize> }
...
fn bench(n: usize, fill: usize, pct: usize, kd: &KeyData) {
    ...
    for r in 0..TOTAL {
        ...
        if r >= WARMUP {
            let idx = r - WARMUP;
            ins[idx] = insert_us; hit[idx] = hit_us; mis[idx] = miss_us; del[idx] = del_us;
        }
    }
...
fn main() {
    println!("=== STOCK hashbrown (std): full parameter sweep ===");
    ...
    println!("\n--- u64 keys (1M, 50% load) ---");
    {
        ...
        for r in 0..TOTAL {
            ...
            if r >= WARMUP {
                let idx = r - WARMUP;
                hit_arr[idx] = hit_us; mis_arr[idx] = miss_us; ins_arr[idx] = ins_us;
            }
        }
