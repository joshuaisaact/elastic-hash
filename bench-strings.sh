...
zig build autobench-strings -Doptimize=ReleaseFast 2>/dev/null

echo "=== Running Abseil (string_view keys) ==="
./bench-abseil-strings > abseil-strings.log 2>/dev/null

echo "=== Running Elastic Hash ([]const u8 keys) ==="
zig build autobench-strings -Doptimize=ReleaseFast 2> elastic-strings.log

echo ""
echo "=== String Key Results (16-byte hex keys, median of 10 runs) ==="
...
while IFS=$'\t' read -r _ an al ai ag ad am; do
    ...
    e_line=$(grep "n=${n_val}" elastic-strings.log | grep "load=${load_val}" | head -1) || true
...
done < <(grep "^RESULT" abseil-strings.log)
