// Flat layout + triangular probing (abseil's approach, our code)
#include "common.h"
...
struct HashTable {
    ...
    // Triangular probing: (base + probe*(probe+1)/2) & mask
    ...
    uint64_t* get(const char* key, size_t key_len) const {
        ...
        size_t max_probe = std::min(bucket_count, MAX_PROBES);
        for (size_t probe = 0; probe < max_probe; probe++) {
            size_t idx = (bucket_base_raw + probe * (probe + 1) / 2) & bucket_mask;
...
int main() {
    run_bench<HashTable>("flat_triangular");
    return 0;
}
